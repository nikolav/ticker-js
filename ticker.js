...
(function (context, name, def, globals) {
  context[name] = def(globals);
})(
  window,   // context
  'ticker', // name
  function (globals, none) { // factory
  ...
  'rc' : 1 / 0};   // (r)epeat(c)ount (Infinity)
  ...
  var pnow    = (function (tmstart, datenow, globals) {
    return ('now' in Object(globals.performance)) ?
      ...
      (tmstart = datenow(), 
      function () { return datenow() - tmstart; });
  ...
  return paste(function (delay, repeatcount) {
  ...
  function newstate (self, arguments_) {
    ...
    'z0' : bind_(zer0cc, self)};                   // async state reset
  }
  ...
  function start_ (setup) {
    ...
    if (!_.on && isfunc(_.cb)) {
      ...
      setup = a.length ? (isplainobj(setup) ? setup : {'args': slice(a, 0)}) : {};
